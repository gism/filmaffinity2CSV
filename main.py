...
def saveTableToCSV():
    ...
    if len(table[0]) == 9:
        ...
        table_beautiful = tabulate(table,
                                   headers=['ID:' + fa.getUserID(), 'Title', 'Year', 'Vote', 'Voted', 'movieCountry',
                                            'movieDirector', 'movieCast', 'movieGenre'], tablefmt='orgtbl')
    else:
        table_beautiful = tabulate(table,
...
def getImdbIdsThread(queue):
    while True:
        ...
        table[index] = [table[index][0], imdbID[0].decode(), table[index][1], table[index][2], table[index][3],
                        table[index][4], table[index][5], table[index][6], table[index][7], table[index][8]]
        if index % 10 == 0:
...
if backuptoimdb:
    ...
    if len(imdbNotFound) > 0:
        ...
        # Write table with format
        table_notFound = tabulate(imdbNotFound,
                                  headers=['ID fa: ' + fa.getUserID(), 'ID imdb', 'Title', 'Year', 'Vote', 'Voted',
                                           'movieCountry', 'movieDirector', 'movieCast', 'movieGenre'],
                                  tablefmt='orgtbl')
